...
import {useLogout} from "../../features/auth/index.js";
...
export default function AppLayout({ children }) {
    ...
    // Ganti page.url.startsWith → location.pathname.startsWith
    ...
    // Hamburger SVG path class helper
    ...
    return (
        <>
            ...
            <main className="w-full max-w-[1920px] box-border grid min-h-[100dvh] ml-auto mr-auto grid-rows-[1fr] grid-cols-[max-content_1fr]">
                ...
                <aside className={[
                    ...
                    <div className={[
                        ...
                        <p className={minifySidebar ? 'hidden' : 'block font-semibold'}>Tugas</p>
                    </div>
                    ...
                    <div className="grid auto-rows-max capitalize overflow-y-auto overflow-x-hidden box-border pr-[4px] max-h-[calc(100dvh_-_var(--height-header)_-_calc(var(--m)_*_2))]">
                        <Link to="/users" data-title="Users" className={determineSidebarActiveLink('users', false)}>
                            <svg className="max-w-[var(--s)] max-h-[var(--s)] fill-[var(--link-color)]">
                                <use href="/sprite.svg#users" />
                            </svg>
                            <span className={minifySidebar ? 'hidden' : 'block text-sm'}>Users</span>
                        </Link>

                        <details className="box-border open:bg-[var(--transthird-primary-color)]">
                            <summary className={determineSidebarActiveLink('purchases', false)} data-title="Purchases">
                                <svg className="max-w-[var(--s)] max-h-[var(--s)] fill-[var(--link-color)]">
                                    <use href="/sprite.svg#purchases" />
                                </svg>
                                <span className={minifySidebar ? 'hidden' : 'block text-sm'}>Purchases</span>
                            </summary>
                            <div className={minifySidebar ? 'hidden' : 'block box-border'}>
                                <Link to="/purchases/suppliers" className={determineSidebarActiveLink('purchases/suppliers')}>Suppliers</Link>
                                <Link to="/purchases/requisition" className={determineSidebarActiveLink('purchases/requisitions')}>Requisitions</Link>
                            </div>
                        </details>

                        <details className="box-border open:bg-[var(--transthird-primary-color)]">
                            <summary className={determineSidebarActiveLink('inventories', false)} data-title="Inventories">
                                <svg className="max-w-[var(--s)] max-h-[var(--s)] fill-[var(--link-color)]">
                                    <use href="/sprite.svg#inventories" />
                                </svg>
                                <span className={minifySidebar ? 'hidden' : 'block text-sm'}>Inventories</span>
                            </summary>
                            <div className={minifySidebar ? 'hidden' : 'block box-border'}>
                                <Link to="/inventories" className={determineSidebarActiveLink('inventories/products')}>Products</Link>
                            </div>
                        </details>

                        <Link to="/sales" data-title="Sales" className={determineSidebarActiveLink('sales', false)}>
                            <svg className="max-w-[var(--s)] max-h-[var(--s)] fill-[var(--link-color)]">
                                <use href="/sprite.svg#sales" />
                            </svg>
                            <span className={minifySidebar ? 'hidden' : 'block text-sm'}>Sales</span>
                        </Link>
                    </div>
            ...
            {/* ── FOOTER ── */}
            <footer className="flex justify-between box-border">
                ...
            </footer>
        </>
    )
